...
const float PIXEL     = 255.0;//ldexpf(1.0, 14);
void ProxGS(
    ...
    )
{
    ...
    for_y : for (int y = 0; y < HEIGHT; y++)
    {
        ...
        {	//printf("FFT real %f \n",float(255.0*fft_result[y][x].real()));
            ...
            if(coe_b[y][x]!=0) {
            	input_data_re=(PIXEL*(fft_result[y][x].real())+2*float(TAU)*coe_a[y][x].real())/coe_b[y][x];
            	input_data_im=(PIXEL*(fft_result[y][x].imag())+2*float(TAU)*coe_a[y][x].imag())/coe_b[y][x];
            }else {
            	input_data_re=0;
            	input_data_im=0;
            }
...
void P2S(eita_t data_in[HEIGHT][WIDTH],cmpxDataIn data_out[HEIGHT][WIDTH]){ //0~1
	for_y : for (int y = 0; y < HEIGHT; y++)
    {
        ...
        {
            #pragma HLS PIPELINE
        	    data_out[y][x]=cmpxDataIn( data_in_t(data_in[y][x].to_int()/float(PIXEL)),0); //input image type casting to FFT domain
